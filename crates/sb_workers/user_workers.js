...
import { getKhulnasoftTag } from 'ext:sb_core_main_js/js/http.js';
...
const NO_KHULNASOFT_TAG_WARN_MSG = `Unable to find the khulnasoft tag from the request instance.\n\
Invoke \`EdgeRuntime.applyKhulnasoftTag(origReq, newReq)\` if you have cloned the original request.`
...
class UserWorker {
	...
	async fetch(req, opts = {}) {
		const tag = getKhulnasoftTag(req);
		...
		if (tag === void 0) {
			console.warn(NO_KHULNASOFT_TAG_WARN_MSG);
		} 
