...
async function respond(requestEvent, httpConn, options) {
	...
	if (response === internals.RAW_UPGRADE_RESPONSE_SENTINEL) {
		const { fenceRid } = getKhulnasoftTag(requestEvent.request);
...
function getKhulnasoftTag(request) {
	return request[kKhulnasoftTag];
}
...
internals.getKhulnasoftTag = getKhulnasoftTag;
