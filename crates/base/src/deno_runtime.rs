...
static KHULNASOFT_UA: Lazy<String> = Lazy::new(|| {
    let deno_version = MAYBE_DENO_VERSION.get().map(|it| &**it).unwrap_or("1.0.0");
    let khulnasoft_version = option_env!("GIT_V_TAG").unwrap_or("0.1.0");
    format!(
        "Deno/{} (variant; KhulnasoftEdgeRuntime/{})",
        deno_version, khulnasoft_version
    )
});
    ...
    pub async fn new(
        ...
        let extensions = vec![
            ...
            deno_fetch::deno_fetch::init_ops::<Permissions>(deno_fetch::Options {
                user_agent: KHULNASOFT_UA.clone(),
...
mod test {
    ...
    async fn test_os_ops() {
        ...
        assert_eq!(
            ...
            "khulnasoft-edge-runtime-0.1.0 (compatible with Deno vUNKNOWN)"
        );
        assert_eq!(
